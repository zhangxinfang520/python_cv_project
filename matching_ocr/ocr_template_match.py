#导入工具类
'''
argsparse是python的命令行解析的标准模块,直接在命令行中就可以向程序中传入参数并让程序运行.
'''
import argparse
import cv2 #格式bgr
import numpy as np
from matching_ocr.myutils import resize,sort_contours
#设置参数
ap = argparse.ArgumentParser()
ap.add_argument("-i","--image",required=True,help='path to in put image')
...
# ref01 = ref.copy().astype(np.uint8)
...
for(i,c) in enumerate(refCnts):
    ...
    roi = ref[y:y + h, x:x + w]
    roi = resize(roi, 57, 88)#cv2.resize(img,(57,88))
    #每一个数字对应一个模板
    digits[i] = roi
